...
var App = {
    mergeRoadPath(roadPath, options) {
        ...
        var newRoadPath = [];
        roadPath = this.uniqueRoadPath(roadPath);
...
fs.readFile('./b1.csv', (err, data) => {

    let roads = JSON.parse(data.toString());
    var newRoad = App.mergeRoadPath(roads);
    newRoad = App.mergeRoadPath(newRoad);
    newRoad = App.mergeRoadPath(newRoad);
    newRoad = App.mergeRoadPath(newRoad);
    newRoad = App.mergeRoadPath(newRoad);

    fs.writeFile('c.json', JSON.stringify(newRoad))
